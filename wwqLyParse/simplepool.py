...
class Pool(object):
    def __init__(self,size=None):
        ...
        if (self.pool_size is not None):
            for i in range(size):
                self.pool_threads.append(threading.Thread(target=self._pool_runner, name="["+str(self)+"-thread-"+str(i)+"]", args=[self.queue]))
    ...
    def spawn(self,call_method, *k, **kk):
        ...
        if (self.pool_size is not None):
            ...
            pool_thread = threading.Thread(target=f, name="["+str(self)+"-thread-"+str(len(self.pool_threads))+"]")
